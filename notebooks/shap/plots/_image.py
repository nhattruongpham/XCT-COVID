...
def image(shap_values: Explanation or np.ndarray,
    ...
    for row in range(x.shape[0]):
        ...
        fig_size = (12, 10)
        fig, axes = pl.subplots(1, 1, figsize=fig_size)
        if true_labels:
            axes.set_title(true_labels[row], **label_kwargs)
        axes.axis('off')
        if len(shap_values[0][row].shape) == 2:
            abs_vals = np.stack([np.abs(shap_values[i]) for i in range(len(shap_values))], 0).flatten()
        else:
        ...
        for i in range(len(shap_values)):
            if labels is not None:
                axes.set_title(labels[row, i], **label_kwargs)
            sv = shap_values[i][row] if len(shap_values[i][row].shape) == 2 else shap_values[i][row].sum(-1)
            axes.imshow(x_curr_gray, cmap=pl.get_cmap('gray'), alpha=0.15,
                                    extent=(-1, sv.shape[1], sv.shape[0], -1))
            im = axes.imshow(sv, cmap=cmap, vmin=-max_val, vmax=max_val)
            axes.axis('off')
    if hspace == 'auto':
        fig.tight_layout()
    else:
        fig.subplots_adjust(hspace=hspace)
    cb = fig.colorbar(im, ax=np.ravel(axes).tolist(), label="SHAP value", orientation="vertical",
                      aspect=fig_size[1] / aspect) # horizontal
