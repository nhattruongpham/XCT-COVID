...
class MaskedModel():
    ...
    def _full_masking_call(self, masks, zero_index=None, batch_size=None):
        ...
        num_varying_rows = np.zeros(len(masks), dtype=np.int32)
        batch_positions = np.zeros(len(masks)+1, dtype=np.int32)
        varying_rows = []
        if self._variants is not None:
            delta_tmp = self._variants.copy().astype(np.int32)
        all_outputs = []
        for batch_ind in range(0, len(masks), batch_size):
            mask_batch = masks[batch_ind:batch_ind + batch_size]
            all_masked_inputs = []
            num_mask_samples = np.zeros(len(mask_batch), dtype=np.int32)
    ...
    def _delta_masking_call(self, masks, zero_index=None, batch_size=None):
        ...
        batch_positions = np.zeros(len(varying_rows)+1, dtype=np.int32)
    ...
    def main_effects(self, inds=None, batch_size=None):
        ...
        masks = np.zeros(2*len(inds), dtype=np.int32)
...
def make_masks(cluster_matrix):
    ...
    indices = np.zeros(int(np.sum(cluster_matrix[:,3])) + M, dtype=np.int32)
